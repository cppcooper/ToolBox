...
{
	///Bottom Left
	vertices[0] = (float)frame_width - ( frame_width / 2.0f );																		/// X
	vertices[1] = (float)frame_height - ( frame_height / 2.0f );																	/// Y
	vertices[2] = 0.0f;																						/// Z
	...
	///Bottom Right
	vertices[5] = (float)frame_width + ( frame_width / 2.0f );
	vertices[6] = (float)frame_height - ( frame_height / 2.0f );
	vertices[7] = 0.0f;
	vertices[8] = ( (float)frame_start_tx + (float)frame_width ) / (float)png_width;
	vertices[9] = 1.0f - ( ( (float)frame_start_ty + (float)frame_height ) / (float)png_height );

	///Top Right
	vertices[10] = (float)frame_width + ( frame_width / 2.0f );
	vertices[11] = (float)frame_height + ( frame_height / 2.0f );
	vertices[12] = 0.0f;
	vertices[13] = ( (float)frame_start_tx + (float)frame_width ) / (float)png_width;
	vertices[14] = 1.0f - ( (float)frame_start_ty / (float)png_height );

	///Top Left
	vertices[15] = (float)frame_width - ( frame_width / 2.0f );
	vertices[16] = (float)frame_height + ( frame_height / 2.0f );
	vertices[17] = 0.0f;
	vertices[18] = (float)frame_start_tx / (float)png_width;
	vertices[19] = 1.0f - ( (float)frame_start_ty / (float)png_height );
}